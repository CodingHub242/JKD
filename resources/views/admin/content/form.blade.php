                    ...
                    <div>
                        ...
                        @elseif($field['type'] === 'file')
                            ...
                            @if($isEdit && optional($item)->{$key})
                                <p class="mt-2 text-xs text-ink-400">Current: <a href="{{ storage_url(optional($item)->{$key}) }}" target="_blank" class="text-brand-300">view</a></p>
                            @endif
                            ...
                            @if($isEdit && is_array(optional($item)->{$key}) && count(optional($item)->{$key}))
                                <div class="mt-2 flex flex-wrap gap-2">
                                    @foreach(optional($item)->{$key} as $img)
                                        <img src="{{ storage_url($img) }}" class="h-16 w-16 rounded object-cover">
                                    @endforeach
                                </div>
                            @endif
