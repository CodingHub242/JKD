            ...
            @foreach($sliders as $i => $slide)
                @php
                    ...
                    $posterUrl = $slide->poster
                        ? (str_starts_with($slide->poster, 'http') ? $slide->poster : storage_url($slide->poster))
                        : '';
                @endphp
            ...
            @forelse($projects as $project)
                <a href="{{ url('/projects/'.$project->slug) }}" class="project-card group reveal">
                    <div class="overflow-hidden">
                        <img src="{{ $project->cover_image ? storage_url($project->cover_image) : asset('slide1.jpg') }}" alt="{{ $project->title }}" class="project-card__img">
                    </div>
                    <div class="p-6">
                ...
                @foreach($testimonials as $t)
                    <div class="surface reveal p-7">
                        ...
                        <div class="mt-6 flex items-center gap-3">
                            @if($t->avatar)<img src="{{ storage_url($t->avatar) }}" alt="{{ $t->name }}" class="h-11 w-11 rounded-full object-cover">@endif
                            <div>
