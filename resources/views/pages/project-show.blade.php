                ...
                @if(count($gallery))
                    <div class="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-3">
                        @foreach($gallery as $image)
                            <img src="{{ storage_url($image) }}" alt="{{ $project->title }}" class="h-40 w-full rounded-xl object-cover">
                        @endforeach
                    </div>
                @endif
            </div>
                ...
                @foreach($related as $p)
                    <a href="{{ url('/projects/'.$p->slug) }}" class="project-card group">
                        <div class="overflow-hidden">
                            <img src="{{ $p->cover_image ? storage_url($p->cover_image) : asset('slide1.jpg') }}" alt="{{ $p->title }}" class="project-card__img">
                        </div>
                        <div class="p-6">
